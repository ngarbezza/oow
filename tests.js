...
function assertEqual(actual, expected) {
  ...
  return { success: result, failureMessage: `Expected ${expected} got ${actual}` };
}
...
suite('messages added to array and string', () => {
  ...
  // count
  test('counting on list', assertEqual([1, 42, 2, 23].count(n => n >= 2), 3));
  test('counting on string', assertEqual("holaaaaa".count(l => l === "a"), 5));
  // filter on string
  test('counting on string', assertEqual("asaoamaeatahaianaga".filter(l => l !== "a"), "something"));
});